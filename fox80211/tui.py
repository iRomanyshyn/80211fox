...
class Application:
    ...
    def _visible(self) -> list[AccessPoint]:
        ...
        return sorted(
            ...
            key=lambda ap: (now - ap.last_seen >= STALE_AFTER, -ap.recent_average(SCAN_AVERAGE_WINDOW, now)),
        )
    ...
    def _draw_scan(self) -> None:
        ...
        for row, ap in enumerate(visible[offset : offset + page_size]):
            ...
            signal = round(ap.recent_average(SCAN_AVERAGE_WINDOW))
            line = f"{signal:4}  {str(ap.channel or '?'):>4}  {str(ap.frequency or '?'):>5}  {ap.bssid:17}  {ap.ssid:25.25}  {age:5.1f}s"
